      ...
      <div className="flex whitespace-nowrap overflow-hidden">
        ...
        <div className="flex animate-marquee min-w-full">
          {MANIFESTO_ITEMS.map((item, i) => (
            <div key={`ticker-1-${i}`} className="flex items-center">
              <span className="text-[11px] font-bold tracking-[0.2em] text-slate-500 mx-6">
                {item}
              </span>
        ...
        <div className="flex animate-marquee min-w-full" aria-hidden="true">
          {MANIFESTO_ITEMS.map((item, i) => (
            <div key={`ticker-2-${i}`} className="flex items-center">
              <span className="text-[11px] font-bold tracking-[0.2em] text-slate-500 mx-6">
                {item}
              </span>
              <span className="w-1.5 h-1.5 rounded-full bg-slate-300" />
